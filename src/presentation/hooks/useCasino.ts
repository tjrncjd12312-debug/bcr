...
export const SESSION_ROTATION_DEFAULTS = {
  /**
   * 연결 성공 후 첫 로테이션까지 / 이후 간격. **잠정값 — 실측 후 확정.**
   *
   * 예전 5분/7분은 2-클라이언트 구조에서 세션이 금방 죽던 시절의 값이다. 브릿지 모드(브라우저가
   * 유일한 클라이언트)에서는 세션이 얼마나 사는지 아직 잰 적이 없다. 짧게 두면 측정 자체가
   * 불가능하고(재본 적 없는 만료보다 우리가 먼저 끊음) 런치만 늘어 403 [G.8] 위험이 커진다.
   * 그래서 측정이 가능할 만큼 길게 두고, 진짜 만료는 반응형 로테이션(소켓 사망 시 즉시 1회)이
   * 받아낸다. 실제 수명이 확인되면 그 값보다 조금 짧게 조정한다.
   */
  firstDelayMs: 1_800_000,
