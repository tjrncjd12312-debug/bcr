...
pub async fn download_client_update(
    ...
    info!("🚀 자가 업데이트 시작: {} (파일명: {})", download_url, file_name);

    // 1. PredictionApi를 통해 파일 다운로드
    let bytes = state.prediction_repository.download_file(&download_url).await?;
    ...
    let mut is_msi = file_name.to_lowercase().ends_with(".msi") || download_url.to_lowercase().ends_with(".msi");
    ...
    if is_msi {
        ...
        #[cfg(windows)]
        return Ok("설치 프로그램이 실행되었습니다. 앱이 종료됩니다. 설치를 완료해 주세요.".to_string());
    }

    // 3. EXE 파일 교체 처리 (기존 로직)
    let current_exe = std::env::current_exe().map_err(|e| format!("현재 실행 경로 획득 실패: {}", e))?;
    ...
    #[cfg(unix)]
    {
        ...
        let mut perrs = fs::metadata(&new_exe).map_err(|e| e.to_string())?.permissions();
